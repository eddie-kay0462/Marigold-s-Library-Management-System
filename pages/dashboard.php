...
try {
   ...
   $totalMembers = $totalStudents + $totalUsers;
   
   // Count active loans
   $activeLoansQuery = "SELECT COUNT(*) as active_loans FROM active_loans WHERE status = 'Active' OR status = 'Overdue'";
...
<body>
   ...
   <div class="dashboard-container">
       ...
       if(isset($_SESSION['success'])) {
           ...
           echo "<script>
               document.addEventListener('DOMContentLoaded', function() {
                   showSuccessMessage('" . htmlspecialchars($successMessage, ENT_QUOTES) . "');
               });
           </script>";
       }
       ...
       if(isset($_SESSION['error'])) {
           ...
           echo "<script>
               document.addEventListener('DOMContentLoaded', function() {
                   showErrorMessage('" . htmlspecialchars($errorMessage, ENT_QUOTES) . "');
               });
           </script>";
       }
       ?>
           ...
           <!-- Overview Section -->
            ...
            <section id="loans" class="dashboard-section">
                <div class="dashboard-header">
                    ...
                    <div>
                        <button class="btn btn-secondary" onclick="updateOverdueLoans()">
                            <i class="fas fa-sync"></i> Refresh Loans
                        </button>
                    </div>
                </div>
...
<script>
// Function to update overdue loans on page load
    ...
    fetch("../pages/loans/loan_handler.php?action=get_active_loans")
        ...
        .then(data => {
            if (data.status === "success") {
                ...
                // If we're on the loans tab, refresh the display
...
document.addEventListener("DOMContentLoaded", function() {
    // Update overdue loans immediately when page loads
    updateOverdueLoans();
    
    // Also update every minute to catch newly overdue loans
    setInterval(updateOverdueLoans, 60000);
});
</script>
</body>
</html>
